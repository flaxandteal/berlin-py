...
use berlin_core::locations_db::{parse_data_files, LocationsDb, parse_data_blocks, parse_data_list};
...
#[pyfunction]
fn load_from_json(blocks: Vec<Vec<String>>, rows: Vec<HashMap<String, String>>) -> PyResult<LocationsDbProxy> {
    let db = {
        let mut errors: Vec<String> = vec!();
        let blocks: Vec<(String, Result<serde_json::Value, _>)> = blocks.par_iter().enumerate().map(|(m, strings)| {
            strings.par_iter().enumerate()
                .map_with(m, |m, (n, string)| (format!("{m}, {n}"), serde_json::from_str::<serde_json::Value>(string)))
        }).flatten().collect::<_>();

        let blocks: Vec<(String, serde_json::Value)> = blocks.into_iter().filter_map(|(loc, value)|
            match value {
                Ok(value) => Some((loc, value)),
                Err(err) => { errors.push(format!("Block {loc}: {}", err.to_string())); None }
            }
        ).collect::<_>();

        if errors.len() > 0 {
            return Err(PyTypeError::new_err(format!("JSON parsing errors:\n{}", errors.join("\n"))));
        }

        let db = match parse_data_blocks(blocks.into_par_iter(), None) {
            Ok(db) => db,
            Err(err) => { return Err(PyTypeError::new_err(format!("JSON parsing errors:\n{}", err.to_string()))); }
        };

        let mut errors: Vec<String> = vec!();
        let rows = rows.iter().enumerate().filter_map(|(n, row)| {
            match (|row: &HashMap<String, String>| {
                let locode = CsvLocode {
                    ...
                    name: match row.get("name") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    name_wo_diacritics: match row.get("name_wo_diacritics") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    subdivision_code: match row.get("subdivision_code") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    status: match row.get("status") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    date: match row.get("date") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    iata_code: match row.get("iata_code") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
                    function: match row.get("function") { Some(str_ref) => (*str_ref).clone(), None => "".to_string() },
        ...
        if errors.len() > 0 {
            return Err(PyTypeError::new_err(format!("LOCODE parsing errors:\n{}", errors.join("\n"))));
        }
